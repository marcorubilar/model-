...
###### Representación esquemática del modelo Sick Sicker y sus probabilidades de transición.
##### Se puede crear una función para hacer esto también
plotmat(round(t(m.P_trt), 3), pos = c(3,1), ###Define la matriz "m.P_trt"  y ubicación de las estrategias
        lwd = 1, box.lwd = 2, 
        cex.txt = 0.8, 
        box.size = 0.1, 
        box.type = "circle", ### tipo de forma e.g. cuadrado, circulo, rectangulo
        box.prop = 0.5,
        box.col = c("green", "yellow","light blue", "Red"), ####color de las cajas o circulos
        arr.length=.1,
        arr.width=.1,
        self.cex = .4,
        self.shifty = .06,
        ...
        main = "Schematic representation of the Sick-Sicker Model", add = FALSE) ### titulo
...
#### 05.4 Proportion of sick in S1 state #####
...
m.ce <- data.frame(Strategy = v.names.str,
                   ...
                   Effect   = v.tu.d)
m.ce
#### 07 Compute ICERs of Sick-Sicker model ####
